...
1. *Three* test cases, `crash1.gft`, and `crash2.gft`, that cause the
...
3. A markdown file, `writeup.md` with a heading called "bugs" explaining the bug
   triggered by each of your three test cases. This write-up must also explain how
   one can achieve a third crash that is giftcard agnostic, that is, that does not
   require a specific gift card to perform.
